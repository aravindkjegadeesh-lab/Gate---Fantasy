...
if not st.session_state.auth:
    ...
    if page == "Dashboard":
        ...
        if not raw_grades.empty:
            st.dataframe(raw_grades.pivot_table(index='student', columns='subject', values='mark', aggfunc='mean').fillna("-"))
        ...
        st.metric("Current Squad Value", f"£{total_val}m", f"{90-total_val}m remaining")
        ...
        if st.button("Submit Request"):
            if len(new_names) == 5:
                ...
                elif diff > 2: st.error("Max 2 transfers.")
                else:
        ...
        if st.text_input("Admin Key", type="password") == ADMIN_PASSWORD:
            tabs = st.tabs(["Approvals", "Round Control", "Apply Scores (Grid)", "User Tools", "Danger Zone"])
            
            with tabs[0]: # APPROVALS
                pending = pd.read_sql("SELECT username, team, pending_team, pending_captain FROM users WHERE pending_team IS NOT NULL", db_conn)
                if pending.empty: st.info("No requests.")
                for _, row in pending.iterrows():
                    with st.expander(f"Review: {row['username']}"):
                        st.write(f"New Team: {row['pending_team']}")
                        if st.button("Approve", key=f"ap_{row['username']}"):
            ...
            with tabs[2]: # GRID SCORING
                st.subheader("Enter Scores for Current Round")
                subs = [s.strip() for s in info['subjects'].replace(",", " ").split() if s.strip()]
                grid_data = pd.DataFrame({'Student': list(player_prices.keys())})
                for s in subs: grid_data[s] = 0.0
                
                edited_df = st.data_editor(grid_data, hide_index=True)
                if st.button("Bulk Apply Scores"):
                    for _, row in edited_df.iterrows():
                        ...
                        for sub in subs:
                            ...
                            if mark > 0:
                                ...
                                db_conn.execute("INSERT INTO score_history (round_name, student, subject, mark, points) VALUES (?,?,?,?,?)", (info['round_name'], student, sub, mark, pts))
                                # Points Distro
                                for u_n, u_t, u_c, u_tc in db_conn.execute("SELECT username, team, captain, tc_active FROM users").fetchall():
                                    if u_t and student in u_t:
                                        mult = 3 if (student == u_c and u_tc == 1) else (2 if student == u_c else 1)
                                        db_conn.execute("UPDATE users SET total_points = total_points + ? WHERE username=?", (pts * mult, u_n))
                    db_conn.commit(); st.success("All scores applied!")
            ...
            with tabs[4]: # DANGER ZONE
                ...
                if st.button("🔥 FULL SYSTEM RESET (DELETE EVERYTHING)"):
                    ...
                    db_conn.execute("DROP TABLE IF EXISTS budget_history")
                    db_conn.commit(); st.warning("Database wiped."); st.rerun()
